<div class="p-6 dark:bg-transparent lg:p-10"
    x-data="deletarProdutoFormHandler()"
    x-effect="produto_id = $store.produto.id; produto_nome = $store.produto.nome;">
    ...
    <div class="mb-6">
        <p class="text-gray-700 dark:text-gray-300 mb-4">
            Tem certeza que deseja deletar o produto <strong x-text="produto_nome" class="text-red-600 dark:text-red-400"></strong>?
        </p>
        <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg p-4">
            <p class="text-sm text-yellow-800 dark:text-yellow-200">
                <strong>Atenção:</strong> Esta ação não pode ser desfeita. O produto será removido permanentemente do sistema.
            </p>
        </div>
    </div>

    <form id="deletarProdutoForm" @submit.prevent="submitDelete">
    ...
    <script>
        function deletarProdutoFormHandler() {
            return {
                mensagem: '',
                sucesso: false,
                produto_id: '',
                produto_nome: '',
                async submitDelete() {
                    const form = document.getElementById('deletarProdutoForm');
                    ...
                    try {
                        ...
                        if (response.ok) {
                            ...
                            this.mensagem = result.message || 'Produto deletado com sucesso!';
                            this.sucesso = true;
                            ...
                            const result = await response.json();
                            this.mensagem = result.message || 'Erro ao deletar produto.';
                            this.sucesso = false;
                        }
                    } catch (err) {
