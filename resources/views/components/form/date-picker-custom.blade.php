@props([
    ...
    'enableTime' => false, // set true to enable time picker
    'time_24hr' => true,
    'defaultDate' => null,
    'label' => null,
    'placeholder' => 'Selecione a data',
    'name' => null,
    // optional custom format; if omitted we'll use Brazilian defaults
    'dateFormat' => null,
])
...
<div x-data="{
    ...
    init() {
        this.$nextTick(() => {
            // determine dateFormat: use provided, otherwise pick BR defaults
            let df = {{ $dateFormat ? json_encode($dateFormat) : 'null' }};
            if (!df) {
                df = {{ $enableTime ? "'d/m/Y H:i'" : "'d/m/Y'" }};
            }
            ...
            this.flatpickrInstance = flatpickr(this.$refs.dateInput, {
                ...
                time_24hr: {{ $time_24hr ? 'true' : 'false' }},
                defaultDate: {{ $defaultDate ? (is_array($defaultDate) ? json_encode($defaultDate) : "'" . $defaultDate . "'") : 'null' }},
                // Portuguese (Brazil) locale embedded so component works without extra imports
                locale: {
                    weekdays: {
