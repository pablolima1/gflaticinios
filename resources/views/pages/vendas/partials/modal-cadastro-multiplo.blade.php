<div class="relative w-full rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
    <h2 class="mb-6 text-lg font-bold text-gray-800 dark:text-white">Cadastro Múltiplo de Vendas/Pedidos</h2>
    <form id="multiVendaForm" class="grid grid-cols-1 gap-4">
        <div id="multi-entries">
            <!-- Aqui serão adicionados dinamicamente os campos para múltiplos cadastros -->
            <div class="mb-4 border-b pb-4 grid grid-cols-1 gap-4">
        ...
        <button type="button" class="bg-gray-200 px-3 py-1 rounded mb-4" onclick="addEntry()">Adicionar Mais</button>
        <div class="flex justify-end mt-2">
            <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded">Confirmar Todos</button>
        </div>
    </form>
    <script>
        function addEntry() {
            const entry = document.querySelector('#multi-entries > div').cloneNode(true);
            entry.querySelectorAll('input').forEach(input => input.value = '');
            document.getElementById('multi-entries').appendChild(entry);
        }
    </script>
</div>
